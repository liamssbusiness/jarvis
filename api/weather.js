...
async function fetchWeather(location, units = 'celsius') {
  const { lat = '51.5074', lon = '-0.1278' } = location || {};
  const parsedLat = parseFloat(lat);
  const parsedLon = parseFloat(lon);
  if (isNaN(parsedLat) || isNaN(parsedLon)) throw new Error('Invalid lat/lon parameters');
...
const handler = async function(req, res) {
  ...
  try {
    const result = await fetchWeather({ lat, lon }, units);
